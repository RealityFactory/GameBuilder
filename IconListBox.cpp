...
void CIconListBox::OnDropFiles(HDROP hDropInfo)
{
...
void CIconListBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	}else if (m_TextPos == ITEM_LEFT )
	{
		...
		}else
		{
			...
			rectIcon = new CRect(m_rectListBoxItem.left+6,m_rectListBoxItem.top,m_rectListBoxItem.left+24,m_rectListBoxItem.bottom);
			rectText = new CRect(m_rectListBoxItem.left+30,m_rectListBoxItem.top,m_rectListBoxItem.right,m_rectListBoxItem.bottom);
		}
...

	{
		...
		if ( m_TextPos == ITEM_LEFT )
		{
			...
			{
				//pDC->DrawIcon(rectIcon->left,rectIcon->top,pListDataItem->hIcon);
				DrawIconEx(pDC->GetSafeHdc( ), rectIcon->left+1, rectIcon->top+1, pListDataItem->hIcon, 16, 16, 0, NULL, DI_NORMAL);
			}
		}
...
void CIconListBox::SetTextPosition(Item_Pos Txt_Pos)
{
	m_TextPos = Txt_Pos;
}

void CIconListBox::SetIconPosition(Item_Pos Ico_Pos)
{
	m_IconPos = Ico_Pos;
}

void CIconListBox::Add(CString strItemName, HICON hDataIcon)
{
